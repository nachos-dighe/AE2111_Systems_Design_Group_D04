...
A_stringer_L = 0.000175  #assumed [m]
A_stringer_I = 0.000275  #assumed [m]
...
A_tot = 4*A_stringer_L + (n_stringer_top+n_stringer_bottom-4)*A_stringer_I
...
def column_buckling(z_LE_0, z_TE_0, I_wb, BM, n_stringer, type_stringer_lst, t):  #z_LE_0,z_TE_0  (0: can be replaced with top/bottom), BM: pos/neg crit,
 
    #stringer material/geometry
    E = 68.9 * 10 ** 9
    L = 5.5  #1.375#5.5 #24.63 / 2  # half-span (w.o. coordinate transformation)
    K = 4 #1 / 4  # K= (1/4) one fixed, one free end  #K = 4 one fixed, one free end
    ...
    for i in range(0, n_stringer):
        if i ==0 or i ==(n_stringer-1):
            Astringer_lst =np.full((len(BM)), A_stringer_L)
        else:
            Astringer_lst =np.full((len(BM)), A_stringer_I)
        ...
        #print('Iwb before:  ',I_wb[0])  #testing #works 
        I_wb += Iminlst + zlst[i]**2*Astringer_lst
        #print('Iwb after:  ',I_wb[0])     #testing #works  
        #failure stress
...
#print('INIT IWB:',I_wb[0]) #testing #works 

#geometric computation: wingbox (all design options)
#no longer needed as stringer area is constant
'''
...
MOS_lst_lst_top, sigma_crit_top_lst, sigma_allow_top_lst = column_buckling(z_LE_top, z_TE_top, I_wb, BMres_poscrit_mod, n_stringer_top, type_stringer_lst_top, t_stringer_top) #does not work
MOS_lst_lst_bottom, sigma_crit_bottom_lst, sigma_allow_bottom_lst  = column_buckling(z_LE_bottom, z_TE_bottom, I_wb, BMres_negcrit_mod, n_stringer_bottom, type_stringer_lst_bottom, t_stringer_bottom)
...
#plot for some span only 
lim = 500
for n in range(0,n_stringer_top):
    label_txt = 'Stringer: ' + str(n+1)
    ax1.plot(ylst_0_mod[:lim],MOS_lst_lst_top[n,:lim], label = label_txt)
ax1.legend()
...
for n in range(0,n_stringer_bottom):
    label_txt = 'Stringer: ' + str(n+1)
    ax2.plot(ylst_0_mod[:lim],MOS_lst_lst_bottom[n,:lim], label = label_txt)
ax2.legend()
