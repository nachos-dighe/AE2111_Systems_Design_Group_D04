...
#WARNING
print('------WARNING------\n Always use *variable_mod*',\
      'syntaxin this program to avoid singular points.\n -------------')
...
def MOI_calculator(DeltaX, beta, alpha, z, b):
    Ixx1_per_t = ((((DeltaX) ** 3) * ((np.sin(beta)) ** 2)) \
                  / (12 * ((np.cos(beta)) ** 3))) + ((DeltaX) / np.cos(beta))\
                  * (z - ((DeltaX * np.tan(beta)) / 2)) ** 2  \
                  # lower left angled profile

    Ixx2_per_t = (((DeltaX) ** 3) * ((np.sin((alpha)) ** 2)) \
                  / (12 * ((np.cos(alpha)) ** 3))) \
                  + (DeltaX / (np.cos(alpha))) * ((-DeltaX * np.tan(beta)) \
                                                  - b - ((DeltaX \
                                                          * np.tan(alpha)) / 2) \
                                                  + z) ** 2  \
                                                  # upper right angled profile

    Ixx3_per_t = 1 / 12 * ((DeltaX * np.tan(beta) + b + DeltaX \
                            * np.tan(alpha)) ** 3) + (
                DeltaX * np.tan(beta) + b + DeltaX * np.tan(alpha)) \
                * (z - ((DeltaX * np.tan(beta) + b + DeltaX * np.tan(alpha)) \
                        / 2)) ** 2  # vertical profile on the left
    #MOI per unit thickness  [m^3]              
    Ixx4_per_t = 1 / 12 * b ** 3 + b * (DeltaX * np.tan(beta) + (b / 2) - z) \
                 ** 2  # vertical profile on the right

    #MOI, corrected for thickness [m^4]
    Ixx = (Ixx1_per_t + Ixx2_per_t)*t_wingbox_side + (Ixx3_per_t + Ixx4_per_t)\
          *t_wingbox_tb
    return Ixx

def column_buckling(z_LE_0, z_TE_0, I_wb, BM, n_stringer, type_stringer_lst,\
                    Astringer_lst, t): \
                    #z_LE_0,z_TE_0  (0: can be replaced with top/bottom), \
                    #BM: pos/neg crit,
 
    #stringer material/geometry
    E = 68.9 * 10 ** 9
    L = 24.63 / 2  # half-span (w.o. coordinate transformation)
    K = 1 / 4  # K= (1/4) one fixed, one free end  #K = 4 one fixed,
               #one free end
    ...
    for i in range(0, n_stringer):
        ...
        if type_stringer_lst[i] ==0: #I-stringer
            ...
            Iminlst =((blst-yclst)**2*alst+yclst**2*clst+(blst/2-yclst)**2\
                      *blst+blst**3/12) #Ixx/t= Imin/t [m^3]
        elif type_stringer_lst[i] ==1: #L-stringer
            alst =  (Astringer_lst/t)/2 #based on optimised design
            blst = alst
            yclst = alst/4 #xclst = yclst
            Iminlst = (yclst** 2 * alst + alst** 3 / 12 \
                       + (alst / 2 - yclst)** 2 * alst) \
                       #Ixx/t = Iyy/t =Imin/t [m^3]
    ...
    sigma_allow= Sf*BM*zlst[i]/(I_wb*(n_stringer+1)) \
                 #compatibility eqn approx: load carried equally by stringers \
                 # + wb skin
...
type_stringer_lst_top = np.array([1,0,0,1])
#np.array([[1,0,0,1], [1,0,1], [1,1]])
#0: I-stringer; 1: L-stringer (come up with better implementation)
type_stringer_lst_bottom = np.array([1,0,1])
#np.array([[1,0,1], [1,1], [1,1]])
#0: I-stringer; 1: L-stringer (come up with better implementation)

#optimisation
A_tot = A_stringer_root_top*n_stringer_top+A_stringer_root_bottom\
        *n_stringer_bottom
...
Astringer_top_lst=np.linspace(A_stringer_root_top, A_stringer_tip_top,\
                              len(ylst_0_mod)) #works
Astringer_bottom_lst=np.linspace(A_stringer_root_bottom, A_stringer_tip_bottom,\
                                 len(ylst_0_mod))#works


#print(n_stringer_top[j], type_stringer_lst_top[j], Astringer_top_lst[j],
#t_stringer_top[j]) #works
#spanwise margin of safety
MOS_lst_lst_top, sigma_crit_top_lst, sigma_allow_top_lst = \
                 column_buckling(z_LE_top, z_TE_top, I_wb, BMres_poscrit_mod, \
                                 n_stringer_top, type_stringer_lst_top, \
                                 Astringer_top_lst, t_stringer_top) \
                        
MOS_lst_lst_bottom, sigma_crit_bottom_lst, sigma_allow_bottom_lst  = \
                    column_buckling(z_LE_bottom, z_TE_bottom, I_wb, \
                                    BMres_negcrit_mod, n_stringer_bottom, \
                                    type_stringer_lst_bottom, \
                                    Astringer_bottom_lst, t_stringer_bottom)
...
ax1.grid(True)    
#ax1.set_yticks(np.linspace(round(np.min(MOS_lst_lst_top)), 
#round(np.max(MOS_lst_lst_top)), 10)) 
...
ax2.set_title('Spanwise Margin of Safety: Bottom Panel Stringers')
ax2.grid(True)
