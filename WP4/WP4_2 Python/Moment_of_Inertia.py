...
#coordinate system: positive y up, positive x right
...
def Iycalculator(l,b,theta,beta,t,x,y):
    ...
    Iy_rectangle_inner = (1/12) * ((l - 2 *t)**3)*(b-2*t) + (l-2*t) * (b - 2*t)*((t+(1/2)*(l-2*t))-x)**2 

    Iy_upper_triangle_inner = (1/36) * ((l-2*t)**3)*(l-2*t)*tan(theta)+((((l-2*t)**2)*tan(theta))/2)*(x-(t+((l-2*t)/3)))**2 
